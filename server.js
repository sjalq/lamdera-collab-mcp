...
class CollabMCP {
  ...
  setupHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, () => ({ tools: [
      ...
      { name: 'create_document', description: 'Create document', inputSchema: { type: 'object', required: ['title', 'type'], properties: { title: { type: 'string' }, content: { type: 'string' }, type: { type: 'string', enum: ['plan', 'specification', 'notes', 'code', 'other'] }, project_id: { type: 'number' }, worker_type: { type: 'string', enum: ['dev', 'pm', 'reviewer'] }, worker_name: { type: 'string' } } } },
      { name: 'update_document', description: 'Update document', inputSchema: { type: 'object', required: ['document_id'], properties: { document_id: { type: 'number' }, title: { type: 'string' }, content: { type: 'string' }, type: { type: 'string', enum: ['plan', 'specification', 'notes', 'code', 'other'] }, worker_type: { type: 'string', enum: ['dev', 'pm', 'reviewer'] }, worker_name: { type: 'string' } } } },
      ...
      try {
        ...
        switch (name) {
          ...
          case 'create_document':
            params = { title: args.title, content: args.content || '', type: args.type, project_id: args.project_id };
            if (Object.keys(workerData).length > 0) params.created_by_worker = workerData;
            break;
          case 'update_document':
            params = { document_id: args.document_id, title: args.title, content: args.content, type: args.type };
            if (Object.keys(workerData).length > 0) params.last_modified_by_worker = workerData;
            break;
